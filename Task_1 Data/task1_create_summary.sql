...
select publication_year,
    book_count,
    book_count - LAG(book_count) over (
        ORDER BY publication_year
    ) AS growth
from dbo.yearly_summary
ORDER BY publication_year;
SELECT TOP 1 publication_year,
    book_count,
    book_count - LAG(book_count) OVER (
        ORDER BY publication_year
    ) AS growth
...
== == == = use books_db IF OBJECT_ID('dbo.yearly_summary', 'U') IS NOT NULL DROP TABLE dbo.yearly_summary;
GO
SELECT year AS publication_year,
    ...
    ) AS average_price_usd INTO dbo.yearly_summary
FROM dbo.books_raw
GROUP BY year;
GO
SELECT *
